---
title: "General Linear Model - Factorial Designs"
...
We then obtain an additive model and the effects of the two treatments can be assessed separately.
...
Here, the interaction was not significant suggesting that the effect if the poison does not change according to the treatment and vice versa. In this case, it would make sense to estimate 
...
There is an extremely significant effect of the Poison and Treatment on the dying rate of the rats (p << 0.001).
...
The dying rate is on average `r confint(comparisonsInt2)$confint[contrastNames=="III-I",1] %>% round(2)`  and `r confint(comparisonsInt2)$confint[contrastNames=="III-II",1] %>% round(2)` higher for rats that are exposed to poison III that those exposed to poison I and II, respectively (both p << 0.001, 95% CI III-I: [`r paste(round(confint(comparisonsInt2)$confint[contrastNames=="III-I",-1],2),collapse=", ")`], 95% CI III-II: [`r paste(round(confint(comparisonsInt2)$confint[contrastNames=="III-II",-1],2),collapse=", ")`])
The average dying rate was not significantly different between rats exposed to poison I and poison II (p=`r round(summary(comparisonsInt2)$test$pvalues[contrastNames=="II-I"],3)`).

The dying rate is on average `r confint(comparisonsInt2)$confint[contrastNames=="B-A",1] %>% round(2) %>% abs` and `r confint(comparisonsInt2)$confint[contrastNames=="D-A",1] %>% round(2)%>% abs` lower upon treatment A than upon treatment B and D, respectively (p << 0.001, 95% CI B-A: [`r paste(round(confint(comparisonsInt2)$confint[contrastNames=="B-A",-1],2),collapse=", ")`], 95% CI D-A: [`r paste(round(confint(comparisonsInt2)$confint[contrastNames=="D-A",-1],2),collapse=", ")`]. 
The dying rate is on average `r confint(comparisonsInt2)$confint[contrastNames=="C-B",1] %>% round(2)` and `r confint(comparisonsInt2)$confint[contrastNames=="D-C",1] %>% round(2) %>% abs` higher upon treatment C than upon treatment B and D, respectively (C-B: p << 0.001, 95% CI [`r paste(round(confint(comparisonsInt2)$confint[contrastNames=="C-B",-1],2),collapse=", ")`] , D-C: p = `r summary(comparisonsInt2)$test$pvalues[contrastNames=="D-C"] %>% round(3)`, 95% CI [`r paste(round(confint(comparisonsInt2)$confint[contrastNames=="D-C",-1],2),collapse=", ")`]). 
The average dying rate was not significantly different between treatment C and A (p = `r summary(comparisonsInt2)$test$pvalues[contrastNames=="C-A"] %>% round(3)`), and, between treatment B and D (p = `r summary(comparisonsInt2)$test$pvalues[contrastNames=="D-B"] %>% round(3)`).
